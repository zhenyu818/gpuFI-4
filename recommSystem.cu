...
int main(int argc, char **argv) {
    ...
    if (!file.is_open()) {
        std::cerr << "Error: Cannot open result.txt" << std::endl;
    ...
    // 准备索引范围
    size_t p_start = 0;
    size_t p_end = p_start + h_P.size();
    size_t q_start = p_end;
    size_t q_end = q_start + h_Q_target.size();
    size_t ub_start = q_end;
    size_t ub_end = ub_start + h_user_bias.size();
    ...
    if (match) {
        for (size_t i = 0; i < h_Q_target.size() && match; ++i) {
            std::string ref_str = ref_tokens[q_start + i];
            float ref_val = parse_float(ref_str);
            float computed = h_Q_target[i];
            if (!compare_floats(computed, ref_val, tolerance)) {
                match = false;
            }
        }
    ...
    if (match) {
        for (size_t i = 0; i < h_user_bias.size() && match; ++i) {
            std::string ref_str = ref_tokens[ub_start + i];
            float ref_val = parse_float(ref_str);
            float computed = h_user_bias[i];
            if (!compare_floats(computed, ref_val, tolerance)) {
                match = false;
            }
        }
    ...
    if (match) {
        for (size_t i = 0; i < h_item_bias_target.size() && match; ++i) {
            std::string ref_str = ref_tokens[ib_start + i];
            float ref_val = parse_float(ref_str);
            float computed = h_item_bias_target[i];
            if (!compare_floats(computed, ref_val, tolerance)) {
                match = false;
            }
        }
    ...
    // item_updated (精确比对 int 0/1)
    if (match) {
        for (size_t i = 0; i < h_item_updated.size() && match; ++i) {
            std::string ref_str = ref_tokens[updated_start + i];
            int ref_int;
            try {
            ...
            int computed = static_cast<int>(h_item_updated[i]);
            if (ref_int != computed) {
                match = false;
            }
        }
