/* Inputs 2:4 structured sparse per row */
#include <assert.h>
#include <cuda.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    assert(pathMatrix);
    // Generate 2:4 per row
    for (unsigned int i = 0; i < numNodes; i++) {
        for (unsigned int j = 0; j < numNodes; j += GROUP) {
            bool keep[GROUP] = {0};
            int cnt = 0;
            while (cnt < KEEP) {
                int idx = rand() % GROUP;
            ...
            for (int k = 0; k < GROUP && (j + k) < (int)numNodes; k++) {
                if (i == j + k) {
                    pathDistanceMatrix[i * numNodes + (j + k)] = 0;
                } else {
                    pathDistanceMatrix[i * numNodes + (j + k)] = keep[k] ? (1 + (rand() % (MAXDISTANCE))) : 0;
                }
            }
        }
    }
