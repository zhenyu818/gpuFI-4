...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < n * d; i++) {
      ...
      if (rand_val < 20) {
          key[i]   = __float2half((float)INT_MAX);
          value[i] = __float2half((float)INT_MAX);
      } else if (rand_val < 40) {
          key[i]   = __float2half((float)INT_MIN);
          value[i] = __float2half((float)INT_MIN);
      } else if (rand_val < 60) {
          key[i]   = __float2half((float)(-(rand() % 100)));
          value[i] = __float2half((float)(-(rand() % 100)));
      } else {
          key[i]   = __float2half((float)(rand() % 10));
          value[i] = __float2half((float)(rand() % 10));
      }
  }
