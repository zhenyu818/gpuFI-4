...
int main(int argc, char **argv) {
    ...
    for (long i = 0; i < num_parameters; i++) {
        // 50% NaN, 50% small number
        params_memory[i] = (rand() % 2 == 0) ? NAN : ((float)(rand() % 10));
        grads_memory[i] = (rand() % 2 == 0) ? NAN : ((float)(rand() % 10));
        // keep m, v as finite values to avoid too-early NaN everywhere
