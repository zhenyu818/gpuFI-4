...
int main(int argc, char **argv) {
    ...
    for (long i = 0; i < num_parameters; i++) {
        ...
        params_memory[i] = r1 * 2.0f - 1.0f; // [-1,1]
        grads_memory[i] = r2 * 2.0f - 1.0f;  // [-1,1]
        m_memory[i] = (float)rand() / RAND_MAX * 2.0f - 1.0f;
        v_memory[i] = (float)rand() / RAND_MAX;
    }
    ...
    long total_elements = 3 * num_parameters;
    float *new_results = (float *)malloc(total_elements * sizeof(float));
    ...
    float *ref_results = (float *)malloc(total_elements * sizeof(float));
    long idx = 0;
    float temp;
    while (idx < total_elements && fscanf(fp, "%f", &temp) == 1) {
        ref_results[idx++] = temp;
    }
    fclose(fp);
