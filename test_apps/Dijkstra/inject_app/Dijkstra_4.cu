// Inject app for Dijkstra Kernel1 (2:4 structured sparse weights)
...
__device__ inline float atomicMin(float *addr, float value) {
    return atomicMinFloat(addr, value);
}
...
static void generate_2to4_sparse_weights(float *w, int E) {
    for (int i = 0; i < E; i += 4) {
        bool pick[4] = {false, false, false, false};
        int cnt = 0;
        while (cnt < 2) {
            int idx = rand() % 4;
            if (!pick[idx]) {
        ...
        for (int k = 0; k < 4 && (i + k) < E; ++k) {
            if (pick[k]) {
                w[i + k] = (float)rand() / (float)RAND_MAX;
            } else {
                w[i + k] = 0.0f;
            }
        }
...
int main(int argc, char **argv) {
    ...
    generate_2to4_sparse_weights(h_weightArray, E);
    ...
    CUDA_CHECK(cudaMemcpy(d_updatingShortestDistances, h_updatingShortestDistances, sizeof(float) * V,
    ...
    CUDA_CHECK(cudaMemcpy(h_updatingShortestDistances, d_updatingShortestDistances, sizeof(float) * V,
    ...
    int count = 0;
    while (count < V && fscanf(f, "%f", &expected[count]) == 1)
        count++;
    fclose(f);
    if (count != V) {
        printf("Fault Injection Test Failed!\n");
