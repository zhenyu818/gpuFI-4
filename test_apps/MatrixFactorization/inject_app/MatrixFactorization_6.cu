...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < h_Q.size(); ++i)
        h_Q[i] = gen_subnormal();
    std::vector<float> h_Q_target = h_Q;
    std::vector<float> h_user_bias(num_users);
    for (int i = 0; i < num_users; ++i)
        h_user_bias[i] = gen_subnormal();
    std::vector<float> h_item_bias(num_items);
    for (int i = 0; i < num_items; ++i)
        h_item_bias[i] = gen_subnormal();
    std::vector<float> h_item_bias_target = h_item_bias;
