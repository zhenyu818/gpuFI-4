...
int main(int argc, char **argv) {
    ...
    std::vector<float> h_data(nnz, 1.0f);
    for (int i = 0; i < nnz; ++i)
        h_indices[i] = i % num_items;

    std::vector<float> h_P(num_users * latent_factors, 1.0f);
    std::vector<float> h_Q(num_items * latent_factors, 1.0f);
    std::vector<float> h_Q_target(num_items * latent_factors, 1.0f);
    std::vector<float> h_user_bias(num_users, 1.0f);
    std::vector<float> h_item_bias(num_items, 1.0f);
    std::vector<float> h_item_bias_target(num_items, 1.0f);
    std::vector<unsigned char> h_item_updated(num_items, 0u);

    std::vector<int> h_random_choice(num_users, 0);

    float global_bias = 1.0f;
    ...
    CUDA_CHECK(cudaMemcpy(d_random_choice, h_random_choice.data(), h_random_choice.size() * sizeof(int),
                          cudaMemcpyHostToDevice));
    {
        std::vector<int> h_item_owner_vec(num_items, std::numeric_limits<int>::max());
        CUDA_CHECK(cudaMemcpy(d_item_owner, h_item_owner_vec.data(), h_item_owner_vec.size() * sizeof(int),
                              cudaMemcpyHostToDevice));
    }
