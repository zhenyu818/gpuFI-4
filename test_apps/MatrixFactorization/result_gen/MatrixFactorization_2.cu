...
__device__ float get_prediction(int factors, const float *p, const float *q, float ub, float ib, float gb) {
    float pred = gb + ub + ib;
...
int main(int argc, char **argv) {
    ...
    const float lp = 0.02f, lq = 0.02f, lub = 0.02f, lib = 0.02f;
    const bool is_train = true;
    CUDA_CHECK(cudaMemcpyToSymbol(config::n_factors, &latent_factors, sizeof(int)));
    CUDA_CHECK(cudaMemcpyToSymbol(config::learning_rate, &lr, sizeof(float)));
    CUDA_CHECK(cudaMemcpyToSymbol(config::P_reg, &lp, sizeof(float)));
    CUDA_CHECK(cudaMemcpyToSymbol(config::Q_reg, &lq, sizeof(float)));
    CUDA_CHECK(cudaMemcpyToSymbol(config::user_bias_reg, &lub, sizeof(float)));
    CUDA_CHECK(cudaMemcpyToSymbol(config::item_bias_reg, &lib, sizeof(float)));
    ...
    for (int i = 0; i < nnz; ++i)
        h_indices[i] = i % num_items;
    std::vector<float> h_data(nnz, 0.0f);
    std::vector<float> h_P(num_users * latent_factors, 0.0f);
    std::vector<float> h_Q(num_items * latent_factors, 0.0f);
    std::vector<float> h_Q_target(num_items * latent_factors, 0.0f);
    std::vector<float> h_user_bias(num_users, 0.0f);
    std::vector<float> h_item_bias(num_items, 0.0f);
    std::vector<float> h_item_bias_target(num_items, 0.0f);
    std::vector<unsigned char> h_item_updated(num_items, 0u);
    std::vector<int> h_random_choice(num_users, 0);
    ...
    CUDA_CHECK(cudaMemcpy(d_random_choice, h_random_choice.data(), h_random_choice.size() * sizeof(int),
                          cudaMemcpyHostToDevice));
    {
        std::vector<int> h_item_owner(num_items, INT_MAX);
        CUDA_CHECK(
