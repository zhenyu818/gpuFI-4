...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nnz; ++i) {
        ...
        h_data[i] = 1.0f + ((float)rand() / RAND_MAX) * 4.0f; // [1,5]
    }
    ...
    gen_sparse_2to4(h_P.data(), h_P.size());
    gen_sparse_2to4(h_Q.data(), h_Q.size());
    h_Q_target = h_Q;
    ...
    gen_sparse_2to4(h_user_bias.data(), h_user_bias.size());
    gen_sparse_2to4(h_item_bias.data(), h_item_bias.size());
    ...
    float global_bias = ((float)rand() / RAND_MAX) * 0.1f; // small finite bias
