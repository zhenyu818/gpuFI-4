...
int main(int argc, char *argv[]) {
    ...
    float *key_float = (float *)malloc(n * d * sizeof(float));
    gen_sparse_2to4(key_float, n * d);
    for (long i = 0; i < n * d; i++) {
        key[i] = __float2half(key_float[i]);
    }
    free(key_float);

    float *value_float = (float *)malloc(n * d * sizeof(float));
    gen_sparse_2to4(value_float, n * d);
    for (long i = 0; i < n * d; i++) {
        value[i] = __float2half(value_float[i]);
    }
    free(value_float);
