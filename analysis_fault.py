...
def parse_log(log_path: str):
    ...
    # 修改点：允许匹配任意 [xxx_FI_WRITER]
    re_writer = re.compile(
        r"^\[.*FI_WRITER\].*?->\s*(\S+)\s+PC=.*\(([^:()]+):(\d+)\)\s*(.*)$"
    )
            ...
            for raw in f:
                ...
                if current_key is not None:
                    ...
                    if m:
                        ...
                        rec["effective"] = True  # 出现 FI_WRITER 即视为有效
                        continue
...
def write_csv(app: str, test: str, components: str, effects, results, all_runs):
    ...
    inst_counts = (
        {}
    )  # (kernel, inst_line, inst_text) -> {'Masked': n, 'SDC': n, 'DUE': n}
    ...
    for run_key in sorted(all_runs, key=sort_key):
        ...
        if eff_rec and eff_rec.get("effective"):
            ...
            inst_line = (
                eff_rec.get("inst_line") if eff_rec.get("inst_line") is not None else -1
            )
    ...
    if os.path.exists(out_path):
        try:
            with open(out_path, "r", encoding="utf-8") as f:
                ...
                has_invalid_cols = set(
                    ["invalid_Masked", "invalid_SDC", "invalid_DUE"]
                ...
                for row in reader:
                    ...
                    if kernel == "invalid_summary":
                        try:
                            if has_invalid_cols:
                                invalid_counts["Masked"] += int(
                                    row.get("invalid_Masked") or 0
                                )
                                invalid_counts["SDC"] += int(
                                    row.get("invalid_SDC") or 0
                                )
                                invalid_counts["DUE"] += int(
                                    row.get("invalid_DUE") or 0
                                )
                            else:
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for kernel, inst_line, inst_text in sorted(
            ...
            total_project_inj = (
                sum(
                    v.get("Masked", 0) + v.get("SDC", 0) + v.get("DUE", 0)
                    for v in inst_counts.values()
                )
                + invalid_counts.get("Masked", 0)
                + invalid_counts.get("SDC", 0)
                + invalid_counts.get("DUE", 0)
            )
            ...
            writer.writerow(
                {
                    "kernel": kernel,
                    "inst_line": (
                        "" if inst_line is None or inst_line < 0 else inst_line
                    ),
                    "inst_text": inst_text,
        ...
        invalid_valid_rate = (
            (invalid_tot / total_project_inj) if total_project_inj > 0 else 0.0
        )
        invalid_sdc_rate = (
            (invalid_counts.get("SDC", 0) / invalid_tot) if invalid_tot > 0 else 0.0
        )
        writer.writerow(
            {
    ...
    total_masked = sum(
        v.get("Masked", 0) for v in inst_counts.values()
    ) + invalid_counts.get("Masked", 0)
    total_sdc = sum(v.get("SDC", 0) for v in inst_counts.values()) + invalid_counts.get(
        "SDC", 0
    )
    total_due = sum(v.get("DUE", 0) for v in inst_counts.values()) + invalid_counts.get(
        "DUE", 0
    )
...
def main():
    ...
    parser.add_argument(
        "--components", "-c", required=True, help="Components identifier", type=str
    )
    ...
    out_path, invalid_counts, totals = write_csv(
        args.app, args.test, args.components, effects, results, all_runs
    )
    ...
    print(
        f"Overall - Masked: {totals['Masked']}, SDC: {totals['SDC']}, DUE: {totals['DUE']}"
    )
